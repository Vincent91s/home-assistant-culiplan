...
- **The sidebar panel no longer collapses to ~150px.** The panel showed its nav bar and about one card, then page background for the rest of the viewport — which read as a black overlay. `:host` in `culiplan-panel.js` set `height: 100%`, but a percentage height only resolves against a parent with a *definite* height, and HA gives `<ha-panel-custom>` none when a panel is registered with `component_name: "custom"` and `embed_iframe: False`. So it resolved to `auto`, the host shrank to content height, and the nested `.fill` iframe collapsed with it — `<ha-panel-custom>`, `<culiplan-panel>` and the iframe all measured 1344x150 in a 1600x1000 viewport. The host is now a flex column at `100vh` (with `100dvh` after it, so mobile browser chrome does not clip the app) and `.fill` grows into the remaining space. Custom panels render full-bleed, so no header offset is subtracted. Not theme-related; reproduced with `selectedTheme: default`.
- **Lovelace resources no longer duplicate on every restart.** `_async_register_lovelace_resources` documented itself as idempotent and did check for already-registered URLs first — but the check could never succeed. `async_items()` is a synchronous `@callback` returning a list, so `await`ing it raised `TypeError`; the fallback then called `async_load(True)`, and that method takes no arguments, so it raised `TypeError` too. Both landed in a bare `except Exception: existing_items = []`, and an empty "already registered" set meant three fresh rows with new UUIDs on every single startup. One instance had reached **1398 resources** — 466 copies each of three cards — firing that many failed JS requests per dashboard load. The collection is now loaded explicitly and read synchronously, and if the existing set cannot be read the integration **registers nothing** rather than assuming the list is empty.
- **Stale and duplicate Culiplan resource rows are now cleaned up on setup.** Dead `/hacsfiles/culiplan/...` entries are removed, and repeated rows for a URL we own are collapsed to one. Scoped strictly to Culiplan-owned URL prefixes — resources you added yourself are never touched.
- **The registered card files now exist.** All three URLs returned 404. They pointed at `/hacsfiles/culiplan/lovelace/cards/dist/<name>.js` on the assumption that HACS mirrors the repo into `<config>/www/community/`; it does not — for an *integration* repo HACS copies `custom_components/<domain>/` and nothing else, so that path was never populated. The bundles now ship inside the integration at `custom_components/culiplan/frontend/cards/` and are served from the integration's own static path as `/culiplan_static/cards/<name>.js`, which works for HACS, manual copies, and the built-in update entity alike. `lovelace/cards/*.ts` remains the source of truth; `pnpm build:cards` emits to the new location.
- **No more blocking-I/O warnings at startup.** HA logged `Detected blocking call to read_text with args (PosixPath('/config/custom_components/culiplan/manifest.json'),) inside the event loop` on every boot. `helpers._build_device_info` and `update._installed_version` each re-read `manifest.json` from the event loop. The version is now read exactly once, at module import in `const.py` (HA imports integration modules in an executor, so that read never touches the loop), and the two duplicate readers in `__init__.py` and `config_flow.py` collapse into it.

### Tests

- 680 passing (was 678). The Lovelace registration tests previously asserted against an *inline re-implementation* of the function using an `AsyncMock` collection that happily accepted `await async_items()` and `async_load(True)` — neither of which the real HA API supports. That is why the bug shipped green. They now call the real function against a fake that mirrors HA's actual `ResourceStorageCollection` contract, and cover: no accumulation across repeated setups, fail-closed on an unreadable collection, stale `/hacsfiles` removal, duplicate collapsing, and non-fatal create/delete failures. New static guards keep `height: 100%` off the panel's `:host` and assert every registered card URL resolves to a file that actually ships.
